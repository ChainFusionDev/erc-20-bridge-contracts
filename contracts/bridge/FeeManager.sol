...
contract FeeManager is Initializable, SignerOwnable {
    ...
    function setFoundationAddress(address _foundationAddress)
        public
        onlySigner
    {
    ...
    function setValidatorFeePool(address payable _validatorFee)
        public
        onlySigner
    {
    ...
    function setValidatorRefundFee(uint256 _validatorRefundFee)
        public
        onlySigner
    {
    ...
    function distributeRewards(address token) public {
        ...
        if (token == NATIVE_TOKEN) {
            ...
            (
                ...
                liquidityRewards,
                foundationRewards
            ) = _calculateRewards(token, totalRewards);

            // solhint-disable-next-line avoid-low-level-calls
            (bool success, ) = address(validatorFeePool).call{
                value: validatorRewards,
                gas: 21000
            }("");
            ...
            (success, ) = address(liquidityPools).call{
                value: liquidityRewards,
                gas: 21000
            }("");
            require(success, "FeeManager: transfer native token failed");

            // solhint-disable-next-line avoid-low-level-calls
            (success, ) = foundationAddress.call{
                value: foundationRewards,
                gas: 21000
            }("");
            require(success, "FeeManager: transfer native token failed");
        } else {
            ...
            require(
                IERC20(token).transfer(foundationAddress, foundationRewards),
                "IERC20: transfer failed"
            );
        }
        ...
        returns (uint256 fee)
    {
        fee =
            validatorRefundFee +
            (tokenFeePercentage[token] * amount) /
            BASE_DIVISOR;

        require(
            fee <= amount,
            "FeeManager: fee to be less than or equal to amount"
        );
    ...
    {
        validatorRewards =
            (validatorRewardPercentage[token] * totalRewards) /
            BASE_DIVISOR;
        liquidityRewards =
            (liquidityRewardPercentage[token] * totalRewards) /
            BASE_DIVISOR;
